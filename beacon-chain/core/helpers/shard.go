...
// CanCrosslink returns true if there's more than 2/3 attestations voted to crosslink shard transition back to the
// beacon chain. It also returns the transition participants if crosslink has succeeded.
func CanCrosslink(beaconState *s.BeaconState, atts []*ethpb.Attestation, committee []uint64) (bool, []uint64, error) {
	...
	for _, a := range atts {
		attestingIndices, err := AttestingIndices(a.AggregationBits, committee)
		if err != nil {
