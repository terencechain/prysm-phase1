...
func OnlineCountDownRoot(countDown []uint64) ([32]byte, error) {
	...
	countDownChunks, err := htrutils.Pack(countDownMarshaling)
	if err != nil {
	...
	countDownRootsRoot, err := htrutils.BitwiseMerkleize(hasher, countDownChunks, uint64(len(countDownChunks)), balLimit)
	if err != nil {
	...
	return htrutils.MixInLength(countDownRootsRoot, countDownRootsBufRoot), nil
}
